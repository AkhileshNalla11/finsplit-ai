...
- "split": use "equal" when a charge is divided evenly among the people who shared it (most food and drinks). Use "proportional" for a FIXED tax/service amount meant to be distributed in proportion to how much each person's food cost; set "sharedBy" to the people it applies to (usually everyone).
- PERCENTAGE charges (e.g. "18% GST", "10% service charge", "15% tip") are special: do NOT compute the amount. Instead of "total", give "rate" (18% → 0.18) and "appliesTo" — the list of OTHER item labels this percentage is charged on (e.g. the hotel and food items, but NOT drinks if the description excludes them). Use the literal label "all" in appliesTo if it applies to the entire bill. Omit "sharedBy" and "split" for percentage items.
